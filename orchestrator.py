...
#working copy of the locked orchestrator for reference
# """
...
"""
Orchestrator – Intelligent Cascading Hybrid (SPEED OPTIMIZED - NO LOGIC REMOVED)
================================================================================
...
1. Smart early exit when core fields found
2. Conditional Stage 2/3 execution
3. Lazy layout analysis (only when needed)
...
# SPEED OPTIMIZATION: Core fields that enable fast path
...
# ============================================================
# MAIN PIPELINE (WITH SPEED OPTIMIZATIONS)
# ============================================================
...
def run_pipeline(image, max_retries=1, debug=False, use_cache=True) -> Dict:
    try:
        ...
        # SPEED OPTIMIZATION 1: Check if we have core fields
        has_core_fields = all(f in stage1 for f in CORE_FIELDS)
        missing1 = _identify_missing_fields(stage1)
        
        if debug:
            print(f"[STAGE1] Extracted {len(stage1)} fields: {list(stage1.keys())}")
            print(f"[STAGE1] Missing {len(missing1)} fields: {missing1}")
            print(f"[STAGE1] Core fields complete: {has_core_fields}")
        
        # SPEED OPTIMIZATION 2: Early exit if we have everything important
        # Skip expensive AI stages when core data is present and only minor fields missing
        if has_core_fields and len(missing1) <= 2:
            if debug:
                print(f"[SPEED] Fast path: Core fields found, skipping Stage 2/3")
                print(f"[SPEED] Only missing: {missing1}")
            
            # Skip to validation with just Stage 1 results
            ocr_conf = ocr.get("avg_confidence", 0.85)
        ...
        # SPEED OPTIMIZATION 3: Smart Stage 2 execution
        # Only run if missing critical fields or many fields missing
        should_run_stage2 = (
            any(f in missing1 for f in CRITICAL_FIELDS) or 
            len(missing1) > 3
        )
        
        if missing1 and should_run_stage2:
            if debug:
                print(f"[STAGE2] Running semantic extraction for: {missing1}")
            ...
            if debug:
                print(f"[SPEED] Skipping Stage 2 - not needed (missing={len(missing1)}, critical_missing={any(f in missing1 for f in CRITICAL_FIELDS)})")
        ...
        # SPEED OPTIMIZATION 4: Smart Stage 3 execution
        ...
        if missing2 and should_run_stage3:
            if debug:
                print(f"[STAGE3] Running layout extraction for: {missing2}")
            
            # SPEED OPTIMIZATION 5: Lazy layout analysis
            # Only compute expensive layout when Stage 3 actually needs to run
            try:
                layout = vision.analyze_layout(processed, ocr)
            except Exception as e:
                if debug:
            ...
            if layout:
                ...
                if debug:
                    print(f"[STAGE3] No layout data available, skipping")
        else:
            if debug:
                print(f"[SPEED] Skipping Stage 3 - all critical fields found")

        # ================= STAGE 4 (FINAL AUTHORITY) =================
        merged = _merge_stage_results(stage1, stage2, stage3)
        
        if debug:
            print(f"[MERGE] Total fields after merge: {len(merged)}")
        ...
        if debug:
            print(f"[FINAL] Validated fields: {len(final_fields)}")
            print(f"[FINAL] Overall confidence: {confidence}")
