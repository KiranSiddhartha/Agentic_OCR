...
POLICY_MAP = {
    "AUTO": [
        "auto", "vehicle", "vin", "motor", "collision", "comprehensive"
    ],
    "ERQ": [
        "earthquake", "california earthquake authority", "seismic"
    ],
    "FIR": [
        "dwelling fire", "dp3", "fire policy"
    ],
    "FLD": [
        "flood", "fema", "sfha", "nfip"
    ],
    "HAZ": [
        "home guard", "home saver", "dwelling package",
        "farm owner", "insured property address"
    ],
    "HO6": [
        "condominium", "unit owner", "ho6", "co-op"
    ],
    "LL": [
        "landlord", "rental dwelling", "rental owner"
    ],
    "HO": [
        "homeowners", "ho3", "coverage a", "coverage b",
        "personal property", "liability"
    ],
    "UO": [
        "unit owner policy", "master policy"
    ],
    "WND": [
        "windstorm", "wind only", "hail only", "hurricane"
    ]
}
...
def classify_policy(lines):
    ...
    for code, keywords in POLICY_MAP.items():
        for kw in keywords:
            if kw in text:
                return code

    ho_hits = sum(1 for x in ["coverage a", "coverage b", "coverage c"] if x in text)
    if ho_hits >= 2:
        return "HO"
...
def get_policy_explanation(code):
    explanations = {
        "AUTO": "Auto insurance policy.",
        "ERQ": "Earthquake coverage.",
        "FIR": "Fire insurance policy.",
        "FLD": "Flood insurance.",
        "HAZ": "Hazard property insurance.",
        "HO6": "Condominium unit owner policy.",
        "LL": "Landlord insurance.",
        "HO": "Homeowners insurance.",
        "UO": "Unit owner insurance.",
        "WND": "Windstorm insurance."
    }
    return explanations.get(code, "Unknown policy type")
