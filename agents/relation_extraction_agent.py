...
Purpose:
- Extract relationships ONLY (no values)
- Uses layout + proximity + document structure
- Provides relational hints for Stage-3 layout agent
- NEVER mutates or creates fields
"""
...
class RelationExtractionAgent:
    """
    Extracts relations in the form:
    (entity_1, relation_type, entity_2, confidence)

    Examples:
    - ("Policy Number", "HAS_VALUE", "12345678", 0.92)
    - ("Effective Date", "PRECEDES", "Expiration Date", 0.90)
    - ("Mortgagee", "CONTAINS", "Loan Number", 0.85)
    """
    ...
    def extract_relations(
        ...
        """
        Extract relations only. No values are created or changed.
        """
        ...
        relations.extend(self._spatial_relations(layout_elements))
        relations.extend(self._key_value_relations(layout_elements))
        relations.extend(self._hierarchy_relations(layout_elements))

        return self._deduplicate(relations)

    # --------------------------------------------------------
    # RELATION STRATEGY 1 — SPATIAL PROXIMITY
    # --------------------------------------------------------

    def _spatial_relations(self, elements: List[Dict]) -> List[Tuple]:
        relations = []

        for i, e1 in enumerate(elements):
            for e2 in elements[i + 1:]:

                dist = self._distance(e1, e2)
                if dist > self.max_distance:
                    continue

                # Below relationship
                if self._is_below(e1, e2):
                    relations.append((
                        e1["text"],
                        "PRECEDES",
                        e2["text"],
                        self._confidence(e1, e2, dist),
                    ))

                # Right-of (key-value)
                if self._is_right_of(e1, e2) and self._looks_like_label(e1["text"]):
                    relations.append((
                        e1["text"],
                        "HAS_VALUE",
                        e2["text"],
    ...
    # --------------------------------------------------------
    # RELATION STRATEGY 2 — FORM KEY/VALUE
    # --------------------------------------------------------

    def _key_value_relations(self, elements: List[Dict]) -> List[Tuple]:
        relations = []

        labels = [e for e in elements if self._looks_like_label(e.get("text", ""))]
        values = [e for e in elements if e not in labels]
        ...
        for label in labels:
            ...
            for value in values:
                score = self._kv_score(label, value)
                if score > best_score:
                    best_score = score
                    best = value
    ...
    # --------------------------------------------------------
    # RELATION STRATEGY 3 — HIERARCHY
    # --------------------------------------------------------

    def _hierarchy_relations(self, elements: List[Dict]) -> List[Tuple]:
        relations = []

        headers = [e for e in elements if e.get("element_type") == "header"]
        headers = sorted(headers, key=lambda e: e["box"][1])

        for i, header in enumerate(headers):
            bottom = header["box"][3]
            top_limit = headers[i + 1]["box"][1] if i + 1 < len(headers) else 1000

            children = [
                e for e in elements
                if bottom < e["box"][1] < top_limit and e != header
            ]
    ...
    def _distance(self, e1: Dict, e2: Dict) -> float:
        c1x, c1y = self._center(e1)
        c2x, c2y = self._center(e2)
        return math.hypot(c2x - c1x, c2y - c1y)

    def _center(self, e: Dict) -> Tuple[float, float]:
        x1, y1, x2, y2 = e["box"]
        return (x1 + x2) / 2, (y1 + y2) / 2

    def _is_below(self, e1: Dict, e2: Dict) -> bool:
        return (
            e2["box"][1] > e1["box"][3]
            and abs(e1["box"][0] - e2["box"][0]) < self.horizontal_threshold
        )

    def _is_right_of(self, e1: Dict, e2: Dict) -> bool:
        return (
            e2["box"][0] > e1["box"][2]
            and abs(e1["box"][1] - e2["box"][1]) < self.vertical_threshold
        )

    # --------------------------------------------------------
    # SCORING (RELATIONS ONLY)
    # --------------------------------------------------------

    def _confidence(self, e1: Dict, e2: Dict, dist: float) -> float:
        base = 0.85
        ocr = (e1.get("confidence", 0.8) + e2.get("confidence", 0.8)) / 2
        dist_penalty = max(0.5, 1 - dist / self.max_distance)
        return round(base * ocr * dist_penalty, 3)

    def _kv_score(self, label: Dict, value: Dict) -> float:
        score = 0.0

        if self._is_right_of(label, value):
            score += 0.6
        if self._is_below(label, value):
            score += 0.3
    ...
    def _looks_like_label(self, text: str) -> bool:
        ...
        patterns = [
            r".*:$",
            r".*(number|date|name|address|premium|policy|insured).*",
        ]
    ...
    def _deduplicate(self, relations: List[Tuple]) -> List[Tuple]:
        best = {}
        for e1, r, e2, conf in relations:
            key = (e1.lower(), r, e2.lower())
            if key not in best or best[key][3] < conf:
                best[key] = (e1, r, e2, conf)
