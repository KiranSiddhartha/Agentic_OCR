# import re
# from classification.document_classifier import classify_document
# from classification.policy_classifier import classify_policy

# # --------------------------------------------------
# # POLICY TYPE KEYWORDS (US + INDIA)
# # --------------------------------------------------
# POLICY_KEYWORDS = {
#     "AUTO": ["auto", "vehicle", "vin", "collision", "motor", "two wheeler", "four wheeler"],
#     "FLOOD": ["flood", "fema", "nfip", "flood zone"],
#     "FIRE": ["fire", "dwelling fire", "dp3", "fire policy"],
#     "HOME": ["homeowner", "ho3", "residential", "home insurance", "householder"],
#     "WIND": ["wind", "hurricane", "cyclone", "hail", "storm"],
#     "EARTHQUAKE": ["earthquake", "earth movement", "seismic"],
#     "HEALTH": ["health insurance", "mediclaim", "hospitalization", "sum insured"],
#     "LIFE": ["life insurance", "policy holder", "sum assured", "nominee"]
# }

# # --------------------------------------------------
# # DOCUMENT TYPE KEYWORDS (US + INDIA)
# # --------------------------------------------------
# DOCUMENT_KEYWORDS = {
#     "COI": ["certificate of insurance", "coi", "acord", "insurance certificate"],
#     "INV": ["invoice", "amount due", "balance due", "tax invoice"],
#     "RNW": ["renewal", "policy renewal"],
#     "CAN": ["cancellation", "cancelled", "rescission"],
#     "RNS": ["reinstatement", "reinstated"],
#     "BIN": ["binder", "cover note", "temporary cover"]
# }
...
# # --------------------------------------------------
# FIELD_RULES = {
#     "COI": ["policy_number", "effective_date", "expiration_date"],
#     "INV": ["invoice_number", "amount_due", "due_date"],
#     "RNW": ["policy_number", "renewal_date"],
#     "CAN": ["policy_number", "cancellation_date"]
# }

# # --------------------------------------------------
# # OCR FIXES (EXACT REPLACEMENTS)
# # --------------------------------------------------
# OCR_FIXES = {
#     # ---------------- POLICY ----------------
#     "pol1cy": "policy",
...
#     "prernlum": "premium",
#     "prem ium": "premium",
#     "annual premiurn": "annual premium",
#     "total premiurn": "total premium",

#     # ---------------- NAME / INSURED NAME ----------------
#     "nane": "name",
#     "narne": "name",
#     "na me": "name",
#     "insured nane": "insured name",
#     "policy holder nane": "policy holder name",
#     "name of insured": "insured name",

#     # ---------------- COMMON OCR GARBAGE WORDS ----------------
#     "athoobhnstanoe": "authorization",
#     "Heonate": "Homeowner",
...
# # --------------------------------------------------
# # INSURANCE DICTIONARY (CORRECT WORDS ONLY)
# # --------------------------------------------------
# INSURANCE_DICTIONARY = {
#     "policy", "insurance", "insured", "insurer",
#     "loan", "number", "policy number",
#     "property", "address",
#     "effective", "expiration", "date",
#     "renewal", "cancellation", "reinstatement",
#     "invoice", "amount", "due", "premium",
#     "certificate", "homeowner", "binder",
#     "flood", "fire", "wind", "earthquake",
#     "health", "life", "mediclaim",
#     "hospitalization", "sum", "insured", "assured",
#     "authorization", "protection", "required"
# }

# # --------------------------------------------------
# # LEVENSHTEIN DISTANCE (NO EXTERNAL LIBS)
# # --------------------------------------------------
# def levenshtein(a, b):
#     if len(a) < len(b):
...
# # --------------------------------------------------
# def correct_word(word, max_distance=2):
...
#     for w in INSURANCE_DICTIONARY:
#         dist = levenshtein(word, w)
#         if dist < best_score:
#             best_score = dist
#             best_match = w

#     return best_match if best_score <= max_distance else word

# # --------------------------------------------------
# # NORMALIZATION PIPELINE (FIXES + DICTIONARY)
# # --------------------------------------------------
# def normalize_text(text):
#     # Step 1: exact OCR fixes
#     for wrong, right in OCR_FIXES.items():
#         text = re.sub(re.escape(wrong), right, text, flags=re.IGNORECASE)

#     # Step 2: dictionary correction
#     corrected_words = []
#     for token in text.split():
#         clean = re.sub(r"[^a-zA-Z]", "", token).lower()
#         corrected = correct_word(clean)
...
#     m = re.search(r"\b[A-Z0-9\/\-]{6,25}\b", text, re.IGNORECASE)
...
#     policy_type = "UNKNOWN"
#     for p, keys in POLICY_KEYWORDS.items():
#         if any(k in clean_text for k in keys):
#             policy_type = p
#             break

#     document_type = "OTHER"
#     for d, keys in DOCUMENT_KEYWORDS.items():
#         if any(k in clean_text for k in keys):
#             document_type = d
#             break
...
#     required_fields = FIELD_RULES.get(document_type, [])

#     if "policy_number" in required_fields:
#         val = extract_policy_number(clean_text)
#         validated["policy_number"] = val or errors.append("policy_number missing")

#     if "effective_date" in required_fields:
#         val = extract_date(clean_text)
#         validated["effective_date"] = val or errors.append("effective_date missing")

#     if "expiration_date" in required_fields:
#         val = extract_date(clean_text)
#         validated["expiration_date"] = val or errors.append("expiration_date missing")

#     if "amount_due" in required_fields:
#         val = extract_amount(clean_text)
#         validated["amount_due"] = val or errors.append("amount_due missing")
...
#     # Desc (CRITICAL for your doc)
#     r"\bdese\b": "desc",
#     r"\bdesce\b": "desc",
    
#     # Insured name
#     r"\binsured\s+nane\b": "insured name",
#     r"\binsured\s+narne\b": "insured name",
# }

# # --------------------------------------------------
# # FUZZY DOCUMENT INTENTS (EXPANDED)
# # --------------------------------------------------
# FUZZY_DOCUMENT_INTENTS = {
#     "RNW": [
#         "renewal notice",
#         "policy renewal",
#         "renew your policy",
...
#         "renewal policy",
#         "transaction desc: renewal policy"  # Exact from your doc
#     ],
...
#     """Lowered to 0.70 for better OCR tolerance"""
#     return SequenceMatcher(None, a, b).ratio() >= threshold


# def fuzzy_intent_detect(text: str, phrases: list) -> bool:
...
#         r"\b(QSN\d{7,})\b",
#         r"\b(QBE\d{7,})\b",
#         r"\b([A-Z]{3}\d{7,})\b"  # Generic 3-letter + numbers
#     ]
...
#     m = re.search(r"loan\s*(?:number|no|#)?\s*[:\-]?\s*(\d{6,})", text, re.I)
#     return m.group(1) if m else None


# def extract_property_address(text):
#     m = re.search(r"(?:property\s*address|address)\s*[:\-]?\s*([a-z0-9 ,\-\.]+)", text, re.I)
#     if m:
#         return m.group(1).strip()

#     m = re.search(r"\b\d{2,5}\s+[a-z0-9\s\.]+(fl|tx|ca|ny|nj|nc|sc|ga)\s*\d{5}\b", text, re.I)
#     return m.group() if m else None
...
#     """Robust insured name extraction"""
#     for i, line in enumerate(lines):
#         l = line.lower()

#         # Check for "customer name" first (specific to invoices)
#         if "customer name" in l or "customer:" in l:
#             parts = re.split(r":", line, maxsplit=1)
#             if len(parts) == 2:
#                 candidate = parts[1].strip()
#                 # Clean up any trailing info
#                 candidate = re.split(r"\s{2,}|policy|invoice", candidate, flags=re.I)[0].strip()
#                 if candidate and len(candidate.split()) >= 2:
#                     return candidate

#         # Standard "insured name" or "named insured"
#         if ("insured" in l and "name" in l) or "named insured" in l:
...
#                     "address", "policy", "number", "date", "mailing", "nfip", "invoice"
#                 ]):
#                     continue

#                 # Check if likely a person name
#                 if len(candidate.split()) >= 2 and len(candidate) <= 60:
#                     return candidate

#     # Fallback: uppercase name blocks
#     for line in lines:
#         if (
#             line.isupper()
...
#                 "policy", "address", "number", "premium", "mortgage", "invoice", "coverage"
#             ])
#         ):
#             return line.strip()
...
#     # Override with fuzzy matching
#     for doc_type, phrases in FUZZY_DOCUMENT_INTENTS.items():
#         if fuzzy_intent_detect(clean_text, phrases):
#             document_type = doc_type
#             break

#     # Special case: Invoice + Renewal = RNW
#     if "invoice" in clean_text and "renewal" in clean_text:
#         if document_type in ["INV", "OTH"]:
#             document_type = "RNW"

#     # Policy inference for FPN
...
#     for field in FIELD_RULES.get(document_type, []):
...
#             val = extract_property_address(clean_text)
